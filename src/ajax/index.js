...
const Ajax = class {
  ...
  addEventListener (xhrId, resolveCb, rejectCb, options) {
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.readyState === 4 && xhr.status === 200) {
        const resConType = xhr.getResponseHeader('content-type')
  ...
  prepareForAjax(options) {
    ...
    const xhrId = Math.random() + ''
    xhrList[xhrId] = {}
    const xhrObj = this.createXHR(options)
  ...
  get (_options) {
    ...
    _options.method = 'GET'
    const { options, xhrObj, promise, resolveCb, rejectCb, xhrId } = this.prepareForAjax(_options)
